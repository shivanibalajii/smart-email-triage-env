...
def grade():
    h = env.get_history()
    if not h:
        return {"total_reward": 0.5, "correct_decisions": 0, "total_steps": 0, "accuracy": 0.5, "score": 0.5}
    ...
    return {
        ...
        "total_steps": len(h),
        "accuracy": round(min(0.99, max(0.01, correct / len(h))), 3),
        "score": round(min(0.99, max(0.01, total / len(h))), 3)
    }
